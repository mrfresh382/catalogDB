...
import os, sys, random , string,  json , httplib2 , requests
...
# ***Needs to be same file name as JSON file downloaded from Google OAUTH API website
# Login.html will need to be updated with your Google API id before running on 
# your local machine. 
APPLICATION_NAME = "Catalog App"

engine = create_engine('sqlite:///catalog-db.db', connect_args={'check_same_thread':False})
...
def gconnect():
  ...
  # Printing user_id info for debug purposes
  try:
    print getUserInfo(login_session['user_id'])
  except:
    pass
...
def gdisconnect():
  ...
  if result['status'] == '200':
    ...
    # Printing remaining login_session data for debug
    for l in  login_session:
      print "login info after logout" + l
...
def showCategorys():
  ...
  # show individual store items for each category
  for category in categorys:
    ...
    indexOfNames += 1
  print listOfItemNames
  if 'username' not in login_session:
    return render_template('publicrestaurants.html', \
      category= categorys, listOfItemNames = listOfItemNames)
  else:
    return render_template('restaurants.html', \
      category= categorys, user_id=login_session['user_id'],\
...
def newCategory():
  ...
  if request.method == 'POST' and login_session['user_id']:
    ...
    to create a category. Please login and stop trying to break in.');} \
...
def editCategory(category_id):
  ...
  if editedCategory.user_id != login_session['user_id']:
    return "<script> function myFunc() { alert('You are not authorized to edit \
    this category. Please create your own category.');}</script><body onload='myFunc()'>"
  if request.method == 'POST' and login_session['user_id']:
...
def deleteCategory(category_id):
  ...
  if categoryToDelete.user_id != login_session['user_id']:
    ...
    Please create your own category.');}</script><body onload='myFunc()'>"
  if request.method == 'POST' and login_session['user_id']:
...
#Show a category list of store items
...
def showList(category_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
      return render_template('publicmenu.html', items= items , category = \
        category , creator = creator, mustLogin = mustLogin)
    else:
...
def newItem(category_id):
  ...
  if category.user_id != login_session['user_id']:
    return "<script> function myFunc() { alert('You are not authorized to create \
    items. Please create your own category.');}</script><body onload='myFunc()'>"
  if request.method == 'POST' and login_session['user_id']:
...
def editItem(category_id, list_id):
    ...
    if request.method == 'POST' and login_session['user_id'] :
        ...
        return render_template('editmenuitem.html', category_id = \
          category_id, list_id = list_id, item = editedItem)
